...
Check if there the species names can be matched with a name on the AMBI species list. By default, the `AMBI()` function will match a family name in the observations without _sp._ with a family name in AMBI where _sp._ is included. By using the `exact_species_match = TRUE` argument only _exact_ matches will be found.
