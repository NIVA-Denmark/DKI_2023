...
Use the `.fix_name()` function from [utils.R](R/utils.R). This will do some simple corrections to species names e.g. replace any *indet.* in species names with *sp.* and remove any double spaces in names.
...
Check if there the species names can be matched with a name on the AMBI species list.

```{r match species AMBI}

species_match <- species %>%
  left_join(ambiR::AMBI_species(), 
            by=c("species"))

```

There are some species names which appear more than once in the AMBI database e.g. *Ophiuroidea sp.* This can result in a warning about *'unexpected many-to-many relationship'* when joining species names from observations with the AMBI database. This can be ignored. It will not cause any problems later.
...
Species names which were not matched with the AMBI species list will be checked with the [WoRMS database](https://marinespecies.org/) using the `.GetSpeciesName()` function from [utils.R](R/utils.R). This can correct the cases where a deprecated species name has been recorded instead of the current recognized name.
...
WoRMS does *not* add *sp.* to species names, so before comparing results from WoRMS with the existing species names, we will *fix* the names as we did previously.
...
Again, any warnings about *many-to-many relationship* can be safely ignored at this stage.

```{r  match species AMBI 2}

species2_match <- species2 %>%
  left_join(ambiR::AMBI_species(), by=c("species"))
...
Using the strictest match specifications (`max_dist = 1`), then `r nrow(species2_replace)` *fuzzy* matches are found:
...
In this case the name *Priapulidae* recorded in the observations was actually the correct nomenclature. However our aim here is to find matches with the AMBI species list.
...
This time we use the original sample nr _Prøvetagningsnummer_ to identify
