...
* [Species data](#species-observations). Check the species names in the observation data for matches with names in the AMBI database. Where possible, correct names which do not match.
* [Salinity](#salinity). Find the salinity information necessary to calculate `DKI`.
* [Calculate DKI](#calculate-dki). Perform the calculations.

# 1. Species observation data

## Read data

Read species observation data downloaded from ODA:

[HentData_WDXSYOYGJE.csv](data/HentData_WDXSYOYGJE.csv).
...
Check if there the species names can be matched with a name on the AMBI species list. By default, the `AMBI()` function will match a family name in the observations without *sp.* with a family name in AMBI where *sp.* is included. By using the `exact_species_match = TRUE` argument only *exact* matches will be found.
...
For species which did have a match, replace the look-up name with the one matched in the AMBI list.
...
For example, we need to remove parts within parenthesis from WoRMS names e.g. *Phascolion (Phascolion) strombus* because the AMBI list does not include these.
...
Using the strictest match specifications (`max_dist = 1`), then `r nrow(species2_replace)` *fuzzy* `r ifelse(nrow(species2_replace)>1,"matches are","match is")` found:
...
species3_match <- species3 %>%
  mutate(count=1) %>%
  ambiR::AMBI(quiet = T)

species3_match <- species3_match$matched %>%
  mutate(AMBI=ifelse(is.na(group),F,T)) %>%
  select(Artsnavn, species, AMBI, AMBI_group=group)

# save the list

write.table(species3_match, file="results/species_names.csv", 
            sep=",", col.names=T, row.names=F, quote=T,
            na="")
...
Arrange the data on each sample date by depth and take the deepest measurement
...
Use the latitude and longitude information in the salinity data to create a spatial data (`sf`) object and then transform to UTM-32
...
For DKI v2, samples are pooled in groups to give a total sampling area of approximately 0.1 m\^2
...
Instead of the original sample nr *Prøvetagningsnummer*, we use the ID for the grouped samples ( *grp_id* ) to identify replicates when calculating AMBI.
...
By default, the name of every species which could not be matched with a name in the AMBI database is listed when calling the `AMBI()` function. Here we have used the `quiet = TRUE` option.
...
Check also any warnings generated. If the total number of species is too small or the total number of individuals, then AMBI should not be used.
...
To calculate DKI from the AMBI results for pooled samples, we need to join them to the salinity information calculated [above](#salinity).
...
This time we use the original sample ID *Prøvetagningsnummer* to identify replicates when calculating AMBI.
...
# 4. Compare pooled and individual
...
ggsave(p, filename="results/compare_DKI.png",
       height=5, width=11, units="in", dpi=300)

```

The dotted grey line in the left-hand figure shows the 1:1 line (DKI for the two methods is equal). The dotted grey line in the right-hand figure shows *Difference = 0*. The red dotted line in the histogram shows the mean difference (**`r format(diff_mean, digits=3)`**)
