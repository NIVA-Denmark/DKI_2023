...
# Read species observation data downloaded from ODA 
# HentData_WDXSYOYGJE.csv
...
# Use the `.fix_name()` function from [utils.R](R/utils.R). This will
# do some simple corrections to species names e.g. replace any *indet.* in # species names and remove any double spaces in names.
...
# Check if there the species names can be matched with a name on the AMBI
# species list. By default, the `AMBI()` function will match a family name
# in the observations without _sp._ with a family name in AMBI where _sp._
# is included. By using the `exact_species_match = TRUE` argument only
# _exact_ matches will be found.
...
species_match <- species_match$matched

# For species which did have a match, replace the look-up name with the 
# one matched in the AMBI list.

species <- species_match %>%
  mutate(species = ifelse(is.na(species_matched), 
                          species, species_matched)) %>%
  select(Artsnavn, species)

### Species names not matched

# Select the species names which were not matched with species 
# names in the AMBI database.
...
# There are **`r nrow(species_unmatched)`** *Artsnavn* which could 
# not be matched with species in the AMBI database. They are listed in this # file: results/species_unmatched.csv

### Check names in WoRMs

# Species names which were not matched with the AMBI species list
# will be checked with the [WoRMS database](https://marinespecies.org/)
# using the `.GetSpeciesNameWoRMS()` function from [utils.R](R/utils.R).
# This can correct the cases where a deprecated species name has 
# been recorded instead of the currently accepted name.
...
# Before comparing results from WoRMS with the existing species 
# names, we will *fix* the names as we did previously.

# For example, we need to remove parts within parenthesis from 
# WoRMS names e.g. _Phascolion (Phascolion) strombus_ because the
# AMBI list does not include these.
...
# Now we will select only the WoRMS results where the species name
# returned by WoRMS is different from the existing species name. 
# If a name we had already matches with the correct name in WoRMS, 
...
# Now we will use the correct names returned by WoRMS to change 
# the table matching `Artsnavn` to `species`
...
species2_match <- species2_match %>%
  ambiR::AMBI(quiet = T)

species2_match <- species2_match$matched
...
# The last thing we can try is to do a *fuzzy* join of species names 
# using the fuzzyjoin package (https://github.com/dgrtwo/fuzzyjoin). 
# This joins species names in observations with species names in AMBI
# which are similar, rather than identical.

# We can limit the *fuzzyjoin* function to return only very similar 
# matches by setting the `max_dist` argument to `1`. The default values 
# is `2`. This will return more possible matches but some of them are 
# likely to be incorrect. For example *Balanidae sp.* (a family of 
# barnacles) is matched with *Maldanidae sp.* (a family of polychaetes).
# In any case, it is a good idea to check the results.
...
# The full list of species names after corrections is here:
# results/species_names.csv
...
# After calculating AMBI, We will use the mean salinity at each
# sampling site to normalize DKI.
...
# Arrange the data on each sample date by depth and 
# take the deepest measurement
...
# Use the latitude and longitude information in the salinity 
# data to create a spatial data (`sf`) object and then 
# transform to UTM-32
...
# Find unique locations for bottom fauna samples and convert
# to spatial data
...
# Now we have a mean salinity value for each observation 
# position in the benthic fauna sampling data
...
# For DKI v2, samples are pooled in groups to give a total
# sampling area of approximately 0.1 m^2
...
# Instead of the original sample nr _Prøvetagningsnummer_, 
# we use the ID for the grouped samples ( _grp_id_ ) to identify 
...
# By default, the name of every species which could not be matched 
# with a name in the AMBI database is listed when calling the
# `AMBI()` function. Here we have used  the `quiet = TRUE` option. 

# Remember to check which species names were not matched. We 
# have already tested our data before calculating AMBI but 
# it is always a good idea to check once more.
...
# Check also any warnings generated. If the total number of 
# species is too small or the total number of individuals,
# then AMBI should not be used.

ambi_res_pooled$warnings

### Add salinity information

# To calculate DKI from the AMBI results for pooled samples,
# we need to join them to the salinity information 
# calculated [above](#salinity-data).
...
# Now we can calculate DKI
  ...
  # Download the results for DKI based on pooled samples:
  results/DKI_pooled.csv
  ...
  # This time we use the original sample ID _Prøvetagningsnummer_ 
  # to identify replicates when calculating AMBI.
  ...
  # As with the grouped samples, we still need salinity 
  # values to calculate DKI.
  ...
  # Download the results for DKI based on individual samples:
  # results/DKI.csv
  
  # Compare pooled and individual
  
  # Compare DKI based on AMBI calculated from individual 
  # samples with DKI from pooled samples
  ...
  # Download the data for the comparison of DKI for pooled 
  # and individual samples:  
  # results/DKI_comparison.csv
  ...
  # The dotted grey line in the left-hand figure shows the
  # 1:1 line (DKI for the two methods is equal). The dotted 
  # grey line in the right-hand figure shows _Difference = 0_. 
  # The red dotted line in the histogram shows the mean difference
